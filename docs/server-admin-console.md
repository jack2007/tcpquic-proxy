...
Relay 页每次刷新**只请求一次** `/api/v1/relay/workers`，不再并行打 `/relay/metrics` 填卡片。

- 卡片来源：从同一响应中 `workers[0]` 且 `worker_id === 'aggregate'` 的行填充 Backend、Active、Pending 和 Errors。
- Worker 表：展示服务端返回的全部行（`aggregate` + 平台 `linux-N` / `darwin-N`；Windows 当前通常只有 `aggregate`）。
- Incomplete：顶层或 aggregate 的 `snapshot_complete:false` 时，页面明确显示 `snapshot incomplete`，不把 partial 的零值当作健康状态。

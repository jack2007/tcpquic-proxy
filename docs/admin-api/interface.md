...
| `GET` | `/api/v1/relay/metrics` | `200` | 返回 relay 聚合指标、`relay_snapshot_complete`、跨平台 `relay_*` 中性字段和平台扩展诊断字段。 |
...
| `GET` | `/api/v1/relay/workers` | `200` | 返回 worker capability 和 worker 列表；首行固定 `aggregate`，Running 时再按 slot 升序追加平台行（Linux `linux-N`、Darwin `darwin-N`）。Windows 的 per-worker `SnapshotWorkers` 尚未落地，当前仍只返回 `aggregate`。 |
| `GET` | `/api/v1/relay/workers/aggregate` | `200`/`404`/`503` | 返回聚合 worker 指标；完整时 `200`，identity 完整但找不到时 `404 not_found`，snapshot 不可用或不完整时 `503 snapshot_unavailable`。 |
| `GET` | `/api/v1/relay/workers/{worker_id}` | `200`/`404`/`503` | 查询 worker 指标。完整时 `200`；identity 完整但 worker 不存在时 `404 not_found`；无法取得 identity 或目标行不完整时 `503 snapshot_unavailable`。 |
...
| `GET` | `/api/v1/relay/active-relays` | `200` | 查询 active relay capability 和当前可见明细列表。 |
| `GET` | `/api/v1/relay/active-relays/{relay_id}` | `200`/`404`/`503` | 查询单个 active relay；当前平台不支持逐 relay 明细时返回 `not_supported`。 |
| `GET` | `/api/v1/relay/workers` | `200` | 返回 worker capability 和 worker 列表；首行固定 `aggregate`，Running 时再按 slot 升序追加平台行（Linux `linux-N`、Darwin `darwin-N`）。Windows 当前仍只返回 `aggregate`。 |
| `GET` | `/api/v1/relay/workers/aggregate` | `200`/`404`/`503` | 返回聚合 worker 指标；完整时 `200`，找不到时 `404 not_found`，snapshot 不可用时 `503 snapshot_unavailable`。 |
| `GET` | `/api/v1/relay/workers/{worker_id}` | `200`/`404`/`503` | 查询 worker 指标；完整时 `200`，找不到时 `404 not_found`，snapshot 不可用时 `503 snapshot_unavailable`。 |
| `POST` | `/api/v1/memory/allocator:dump` | `200` | 将 allocator 统计写到日志，并返回统计 JSON。 |
...
- 中性 runtime snapshot / execution-gate 诊断：`relay_runtime_snapshot_inflight`、`relay_runtime_snapshot_inflight_max`、`relay_runtime_snapshot_acquire_count`、`relay_runtime_snapshot_failure_count`、`relay_runtime_snapshot_stop_wait_count`、`relay_runtime_snapshot_stop_wait_nanos`、`relay_snapshot_execution_busy`、`relay_snapshot_execution_outstanding`、`relay_snapshot_execution_wait_count`、`relay_snapshot_execution_wait_nanos`、`relay_snapshot_execution_deadline_timeouts`、`relay_snapshot_execution_detached_late_commands`。
- Linux 仍额外输出 `linux_relay_*` legacy aliases（与中性 `relay_*` 并存）；调用方应优先读 `relay_*`。
...
`GET /api/v1/relay/workers` 契约：

- 顺序：首行固定 `worker_id=aggregate`，其后按平台 worker slot 升序追加 `linux-N` / `darwin-N`（`N` 从 0 起）。Windows per-worker 行尚未落地，当前只返回 `aggregate`。
- 单次采样：`aggregate` 由同批 per-worker 行归并，不是第二次独立采样。Linux/Darwin 的 `backend` 分别为 `epoll` / `kqueue`。
- `snapshot_complete`：顶层表示整次采样是否完整；每行也有自己的 `snapshot_complete`。Stopped runtime 只返回一行 complete 的零值 `aggregate`。runtime lock / execution permit / worker command 超时导致 identity 或数值不完整时，**list 仍返回 HTTP 200**，但顶层（及不完整行）为 `snapshot_complete:false`；调用方不得把 partial 数值当作完整健康快照。
- aggregate 归并：`active_relays` / `pending_bytes` / `tcp_read_bytes` / `tcp_write_bytes` / `errors` 对同批 worker 行做 **sum**；`event_queue_capacity` 取 **max**。这是顺序采样后的 reduce，不是跨 worker 的全局原子瞬时值。
- capabilities 保持不变：`worker_detail:true`、`per_worker_active_relays:false`、`active_relay_detail` 仍按平台能力报告。

`GET /api/v1/relay/workers/{worker_id}`（含 `/aggregate`）返回 worker 指标，并包含 `relays` 数组。Linux/Darwin worker 对象都包含真实 `event_queue_capacity`（Darwin 为 ring 归一化后的容量）。当前 `per_worker_active_relays:false`，因此 **`relays` 始终为空数组 `[]`**。路由三态：

- 目标行与整体快照都完整：`200`；
- identity 已完整但 `worker_id` 不存在：`404 {"error":{"code":"not_found","message":"not found"}}`；
- 无法取得 identity，或命中行/`aggregate` 的 snapshot 不完整：`503 {"error":{"code":"snapshot_unavailable","message":"relay worker snapshot is unavailable"}}`。

...
| `GET` | `/api/v1/relay/workers` | `200` | 返回 worker capability 和 worker 列表；首行固定 `aggregate`，Running 时再按 slot 升序追加平台行（Linux `linux-N`、Darwin `darwin-N`、Windows `windows-N`）。 |
...
- 顺序：首行固定 `worker_id=aggregate`，其后按平台 worker slot 升序追加 `linux-N` / `darwin-N` / `windows-N`（`N` 从 0 起）。

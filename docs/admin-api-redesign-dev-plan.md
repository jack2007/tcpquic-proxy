...
- `/api/v1/health`、`/api/v1/metrics` 测试通过；旧 `/health`、`/metrics` 返回 `404`。
- Admin 启停不会泄漏线程。
...
- 正 token 返回 `200`。
- 旧路径返回 `404`。
- 所有公开 Admin API 都位于 `/api/v1/*` 并要求 token。
...
| 风险 | 缓解 |
|------|------|
| Admin 重构影响现有脚本 | 文档列出 `/api/v1/*` 替代路径，旧路径直接返回 `404` |

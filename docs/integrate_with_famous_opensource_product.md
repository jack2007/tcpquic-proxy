...
- 客户端验证服务端证书、ALPN、server 侧 CIDR ACL。
...
| 安全穿透 | 使用 QUIC/TLS + 服务端证书校验 + ACL 将 rsync daemon、SFTP、对象存储 API、Dragonfly 组件等服务暴露面收敛到 tcpquic-proxy server 之后。 |
...
- 不能直接开放 rsync/SSH 端口的环境，通过 QUIC/UDP + TLS 服务端身份校验暴露单一入口。
...
- tcpquic-proxy 当前仓库 README、`docs/config_guide_cn.md`、`docs/tls-cert.md`、`docs/system-test-design-tcpquic-proxy_cn.md`

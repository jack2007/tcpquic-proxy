...
也就是说，single-peer 和 multi-peer 都是每个 client runtime manager 一个共享 ingress reactor，不是每个 peer 或每条 QUIC connection 一个独立重连线程。对应 peer 至少有一个 QUIC connection 处于 connected 状态时才开放 ingress listen 端口；全断开或从未连接成功时，该 peer 的 SOCKS5 / HTTP CONNECT listen 端口保持关闭。

...
这些控制面机制是 Linux / Windows / macOS 的公共实现。平台差异只在底层 reactor 和 relay worker 后端；功能开发完成后，三个平台分别做构建、单测和回归验证即可。
...
Windows / macOS 旧路径已经被替换：client 不再为每个 peer 创建独立监听线程和有界握手 `TqThreadPool`；server 普通 OPEN 不再为每条连接创建 detached 拨号线程。旧 fallback 只作为未配置 reactor 时的兼容路径存在，不是当前 Windows / macOS runtime 的主路径。

## Client Ingress Reactor

Client 模式下，进程使用一个 `TqClientIngressReactor` 线程统一处理本地入口。它覆盖 single-peer 和 multi-peer 两种运行方式：所有 peer 的 SOCKS5 listen socket 和 HTTP CONNECT listen socket 都注册到同一个 ingress reactor，而不是每个 peer 各自启动监听线程和握手线程池。
...
- 执行 `QuicClientSession` 注入的 delayed retry task，用于同步 `ConnectionOpen` / `ConnectionStart` 失败后的固定延迟重试。
...
Client 不再有每个 peer 一个独立重连线程，也不再有可配置的重连间隔。异步 QUIC 断开由 MsQuic connection callback 通知；应用在 `QUIC_CONNECTION_EVENT_SHUTDOWN_COMPLETE` 后清理旧 slot 并立即重建同一 slot。同步 `ConnectionOpen` / `ConnectionStart` 失败不会在调用栈内无限立即重试，而是通过注入到 `QuicClientSession` 的 `DelayedTaskScheduler` 投递到 `TqClientIngressReactor::EnqueueDelayed()`，由共享 ingress reactor 线程执行固定延迟 retry。
...
multi-peer client 的 runtime adapter 持有一个共享 `TqClientIngressReactor`。每个 peer 启动自己的 `QuicClientSession`，但不再创建自己的监听线程或握手线程池。peer connected count 大于 0 时，adapter 将该 peer 的 listen fd 注册到共享 reactor；peer drain、删除或断连时，移除该 peer 的 listen fd 和还在握手 / OPEN 阶段的 client socket。

也就是说，single-peer 是一个 client runtime 一个 ingress reactor；multi-peer 是每个 client runtime adapter 一个共享 ingress reactor，不是每个 peer 或每条 QUIC connection 一个独立重连线程。对应 peer 至少有一个 QUIC connection 处于 connected 状态时才开放 ingress listen 端口；全断开或从未连接成功时，该 peer 的 SOCKS5 / HTTP CONNECT listen 端口保持关闭。
...
其中 `P` 是 active peer 数。multi-peer 下 `P` 会影响 `QuicClientSession` 数量，但不会再生成随 peer 线性增长的重连线程、监听线程或握手线程池。同步 start 失败的 retry 由共享 `TqClientIngressReactor` delayed task 队列承载。
...
Linux / Windows / macOS 上的旧监听线程、握手线程池和 per-OPEN 拨号线程路径均不再是主路径；当前三平台 client 和 server 控制面都走跨平台 reactor 模型。

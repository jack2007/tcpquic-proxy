...
| client Connections | `/api/v1/peers/{peer_id}/connections` | 不展示连接成功时间、断开时间、连接累计字节数 |
...
- 详情抽屉宽度需要容纳 paths、port_forwards、listen 字段和 JSON 预览，桌面宽度下不应显得拥挤。
- `Create peer` 打开和 `Edit` 相同结构的详情抽屉，字段顺序固定为：`peer_id`、peer endpoint、`socks_listen`、`http_listen`、`port_forwards`、`enabled`。

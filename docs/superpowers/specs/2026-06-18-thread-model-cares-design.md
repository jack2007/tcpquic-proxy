...
- 推荐最低 c-ares 版本：1.34.0，用于 `ares_process_fds()`。
- 若发行版只能提供更低版本，代码需要编译期检测并使用 `ares_process_fd()` 兼容路径。
...
如果后续选择 IOCP，需要明确使用 `AcceptEx`、overlapped connect、c-ares socket callback 到 IOCP 的桥接方式。这个复杂度高于 Linux，不放进第一阶段。
...
- 优先 vendoring 到 `third_party/c-ares`，保持可重复构建。

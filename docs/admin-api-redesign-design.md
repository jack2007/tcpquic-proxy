...
当前 Admin HTTP API 由 `TqAdminHttpServer` 自研实现，使用阻塞 socket、1 个 accept thread 和每个连接 1 个 detached client thread。现有能力较窄：

- single-peer client：`GET /health`、`GET /metrics`。
- multi-peer client：`GET /health`、`GET /metrics`、`GET /config`、`PUT /config`、`POST /peers/{peer_id}/enable`、`POST /peers/{peer_id}/disable`。
- server：`GET /health`、`GET /metrics`。
...
| 平台 | 默认路径 |
|------|----------|
| Linux/macOS | `$XDG_RUNTIME_DIR/tcpquic-proxy/admin-${pid}.json` |
| Linux/macOS fallback | `/tmp/tcpquic-proxy-$UID/admin-${pid}.json` |
| Windows | `%LOCALAPPDATA%\tcpquic-proxy\admin-${pid}.json` |
...
默认要求 token。若需要短期兼容未鉴权旧脚本，可加显式开关：

```text
--admin-allow-unauthenticated-legacy
```

该开关只影响旧路径，不影响 `/api/v1/*`。启用时仍必须绑定 loopback，并在 stderr 打印高风险警告；非 loopback 绑定不得允许 unauthenticated legacy mode。
...
Phase 1 保留旧 API，并将旧 API 映射到新 service：

| 旧接口 | 新接口 |
|--------|--------|
| `GET /health` | `GET /api/v1/health` |
| `GET /metrics` | client/server 对应 metrics |
...
README 中应标注旧接口 deprecated，但不立即删除。

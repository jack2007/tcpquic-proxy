...
| 平台 | 默认路径 |
|------|----------|
| Linux/macOS | `$XDG_RUNTIME_DIR/<binary-name>/<role>-admin-${pid}.json` |
| Linux/macOS fallback | `/tmp/<binary-name>-$UID/<role>-admin-${pid}.json` |
| Windows | `%LOCALAPPDATA%\<binary-name>\<role>-admin-${pid}.json` |
...
`<binary-name>` 使用进程启动时 `argv[0]` 的 basename；Windows 下会去掉 `.exe` 后缀。例如直接运行 `raypx2` 或 `raypx2.exe` 时均为 `raypx2`，如果用其它文件名或软链接名启动，则跟随实际启动名。

`<role>` 根据启动命令的运行角色生成，取值为 `client` 或 `server`。

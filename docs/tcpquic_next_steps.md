...
- 旧 `TqBlockingDemoRelay` / `TqTcpWriteQueue` 仅存在于 demo/legacy target，供对比测试，不参与 `tcpquic-proxy` 生产链接。
...
- worker 池化缓冲预算与大解压输出之间的关系（demo/legacy `TqTcpWriteQueue` 上限仅影响对比 target）。
...
4. 将 worker 池化缓冲与 fairness budget 纳入 tuning 和 `--max-memory-mb` 预算。

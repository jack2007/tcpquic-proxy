...
- 每条已建立隧道有 TCP->QUIC relay thread。
- QUIC->TCP 方向使用 `TqTcpWriteQueue` writer thread，避免 MsQuic callback 线程阻塞写 TCP。
- 隧道清理由全局 `TqTunnelReaper` 统一回收，避免每隧道 cleanup watcher。

这个模型已经显著降低了历史 per-tunnel watcher 成本，并通过 100 并发验证；但若目标扩展到 500/1000 隧道，per-tunnel relay/writer thread 仍可能成为主要资源瓶颈。
...
- per-tunnel relay thread + writer thread 的线程数和栈内存。
- `TqTcpWriteQueue` 固定 chunk/byte 上限与大解压输出之间的关系。
...
目标：为 500/1000 隧道级别扩展做准备，但避免一开始就大规模重写 relay。

建议阶段：

1. 先加观测，不先重构。
2. 记录 active tunnels、thread count、RSS、relay pool bytes、writer queue depth、in-flight sends、per-tunnel throughput。
3. 将 server 侧 detached dial thread 改为 bounded dial pool。
4. 将 `TqTcpWriteQueue` 的 chunk/byte 上限纳入 tuning 和内存预算。
5. 跑 100/500/1000 并发阶梯测试。
6. 只有当数据证明 per-tunnel thread 是瓶颈时，再设计 epoll/io_uring relay worker 模型。

建议验收：

- 100 隧道无回归。
- 500 隧道资源曲线可解释，无 OOM、无明显 thread explosion。
- 若 1000 隧道失败，能从指标定位是线程、内存、writer queue、TCP 后端还是 QUIC 限制。

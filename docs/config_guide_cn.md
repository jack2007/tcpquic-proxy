...
```jsonc
{
  ...
  "proto": {
    ...
    // 默认值为 true；设为 false 或使用 --enable-encrypt 可启用包加密。
    // client 启用加密不需要配置 client cert/key，只需要 tls.ca 验证 server 证书。
  ...
  "client": {
    // 上报给 server console 的 client 展示名称；只用于展示，不作为认证身份。
    // 未配置时自动使用 <os>-<hostname>，其中 os 为 win、linux 或 macos。
    "client_name": "linux-office-a",
...
```jsonc
{
  ...
  "server": {
    ...
    // server 目标访问 ACL。省略时 CLI/runtime 默认补为 0.0.0.0/0。
    // 生产配置应显式设置最小 CIDR 白名单。
...
| `admin.token_file` | client/server | Admin Bearer token JSON 文件路径。设置后，已有合法 token JSON 会跨重启复用；文件不存在则创建；文件不合法则启动失败。省略时使用运行时默认 token 文件，并且每个进程重新生成。 |
...
| `client.client_name` | client | 上报给 server console 的 client 展示名称；只用于展示，不作为认证身份。允许字母、数字、`.`、`_`、`-`、`:`，最长 64 字节。未配置时自动使用 `<os>-<hostname>`，其中 `os` 为 `win`、`linux` 或 `macos`。 |
| `server.proto_listen` | server | 协议监听端点。server 模式必填。支持逗号分隔 `ip:port` 列表；`0.0.0.0:port` 会展开为本机非本地 IPv4 地址并分别启动 listener。 |
| `server.allow_targets` | server | server 访问 ACL，支持数组或逗号分隔字符串。省略时默认 `0.0.0.0/0`；生产配置应显式设置。 |
...
| `relay.common.read_chunk_size` | client/server | 推荐使用的平台中性 relay TCP read chunk 大小。适用于 Linux/Darwin 等共享 relay 数据面。 |
| `relay.common.tcp_write_max_bytes` | client/server | 推荐使用的平台中性每次 relay TCP `sendmsg` 字节上限；不需要 override 时省略。 |
| `relay.common.tcp_write_burst_bytes` | client/server | 推荐使用的平台中性每轮 TCP write flush 字节上限；不需要 override 时省略。 |
| `relay.common.event_queue_capacity` | client/server | 推荐使用的平台中性 relay 事件队列容量。 |
| `relay.common.worker_count` | client/server | 推荐使用的平台中性 relay worker 数量。 |
| `relay.linux.read_chunk_size` | client/server | 旧版兼容字段；新配置优先使用 `relay.common.read_chunk_size`。 |
| `relay.linux.worker_slots` | client/server | 旧版兼容字段，当前已废弃并忽略；新配置不要继续使用。 |
| `relay.linux.tcp_write_max_bytes` | client/server | 旧版兼容字段；新配置优先使用 `relay.common.tcp_write_max_bytes`。 |
| `relay.linux.tcp_write_burst_bytes` | client/server | 旧版兼容字段；新配置优先使用 `relay.common.tcp_write_burst_bytes`。 |
| `relay.linux.event_queue_capacity` | client/server | 旧版兼容字段；新配置优先使用 `relay.common.event_queue_capacity`。 |
| `relay.linux.worker_count` | client/server | 旧版兼容字段；新配置优先使用 `relay.common.worker_count`。 |
...
## Server Admin ACL 热更新

server 模式启用 admin 后，可以通过 `PATCH /api/v1/server/config` 或 admin-console 的 ACL 页面热更新 `server.allow_targets` 和 `server.deny_targets`。更新成功后，新建普通 tunnel 会立即使用新的 ACL，并且会把配置写回 `--config` 指向的 JSON 文件；写回使用 `nlohmann::json` 读改写 `server.allow_targets` / `server.deny_targets`，不会手工拼接 JSON 字符串。

如果进程不是通过可持久化配置文件启动，PATCH 会返回 `503 not_supported`。启动配置中省略 `server.allow_targets` 仍会默认补 `0.0.0.0/0`；但 admin PATCH 显式提交空 `allow_targets: []` 表示拒绝所有普通新建 tunnel。

## Admin 观测

Admin client connection JSON 会包含每个 QUIC connection slot 的 path 信息。未配置 `paths` 时这些字段为空或表示默认 peer；配置 `paths` 后可通过 `path`、`local`、`peer` 判断 slot 绑定到哪条本地到远端路径。
...
server 默认 `proto.encryption_policy: "client-choice"`，每条入站连接的实际加密状态可通过 Admin API `/api/v1/server/connections` 或 Admin Console 的 server connections 表查看；字段为 `encryption`，取值为 `enabled` 或 `disabled`。

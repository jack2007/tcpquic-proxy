  ...
  "peers": [
    {
      ...
      "id": "dgx-b",
      // 上报给 server console 的展示名称；只用于展示，不作为认证身份。
...
| `peers[].client_name` | client | 上报给 server console 的展示名称；只用于展示，不作为认证身份。允许字母、数字、`.`、`_`、`-`、`:`，最长 64 字节。未配置时多 peer 模式使用 `peer_id`/`id`，单 peer CLI 模式使用 `primary`。 |

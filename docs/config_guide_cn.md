...
{
  "tls": {
    // MsQuic TLS 使用的 server 证书、私钥和 CA 证书。
...
| 配置项 | 模式 | 作用 |
| --- | --- | --- |
| `tls.cert` | client/server | TLS 证书 PEM 路径。必填。 |
| `tls.key` | client/server | TLS 私钥 PEM 路径。必填。 |
| `tls.ca` | client/server | CA 证书 PEM 路径。必填。 |
| `admin.listen` | client/server | admin HTTP 监听地址。 |
| `proto.profile` | client/server | `max-throughput` 或 `low-latency`。 |
| `proto.disable_1rtt_encryption` | client/server | 不安全的 QUIC 1-RTT 加密禁用开关，仅用于实验室测试。 |
...
| `proto.iw` | client/server | QUIC 初始拥塞窗口，单位 packets。 |
| `proto.initrtt_ms` | client/server | QUIC 初始 RTT 估计。 |
...
| `tuning.mode` | client/server | `auto`、`lan`、`wan` 或 `custom`。 |
| `tuning.target_bandwidth_mbps` | client/server | BDP 调优目标带宽。 |
| `tuning.target_rtt_ms` | client/server | BDP 调优目标 RTT。 |
| `tuning.max_memory_mb` | client/server | relay 内存池预算。 |
| `relay.io_size` | client/server | relay IO chunk 大小 override。 |
| `relay.inflight_bytes` | client/server | relay 目标 in-flight 字节数。 |
...
| `trace.connect_on_start` | client | debug 选项，启动时主动建立协议连接。 |
| `peers[].id` | client | 稳定 peer id。必填。 |
| `peers[].proto_peer` | client | 远端协议端点。必填。 |
| `peers[].socks_listen` | client | 该 peer 的 SOCKS5 监听地址。必填。 |
...
- 依赖 `relay.linux.worker_slots`、`proto.fcw`、`proto.initrtt_ms` 等显式 override 时，需要设置 `tuning.mode: "custom"`。
- 内置 speed-test 配置要求只有一个 enabled peer。
- `proto.disable_1rtt_encryption` 不安全，只应用于隔离实验室测试。

...
static const char* TestCallbackTypeName(TqWindowsIocpOperationType type) {
    ...
    case TqWindowsIocpOperationType::QuicPeerSendShutdown:
        return "QuicPeerSendShutdown";
    case TqWindowsIocpOperationType::QuicShutdownComplete:
        return "QuicShutdownComplete";
    case TqWindowsIocpOperationType::TerminalShutdownSinkDrain:
        return "TerminalShutdownSinkDrain";
...
bool IsCallbackSourcedOperation(TqWindowsIocpOperationType type) {
    return type == TqWindowsIocpOperationType::RelayReceiveReady ||
           ...
           type == TqWindowsIocpOperationType::QuicPeerSendShutdown ||
...
struct TqWindowsRelayWorker::WindowsStreamRelayBinding final
    ...
    std::atomic<bool> TerminalShutdownPosted{false};
    uint64_t RelayId{0};
    uint64_t Generation{0};
...
struct TqWindowsRelayWorker::WindowsShutdownSinkTarget final : TqStreamLifetime::Target {
    struct Context {
        std::shared_ptr<TqRelayStopControl> StopControl;
        uint64_t ControlGeneration{0};
    };

    std::shared_ptr<Context> SinkContext;

    explicit WindowsShutdownSinkTarget(std::shared_ptr<Context> context)
        : SinkContext(std::move(context)) {}
...
struct TqWindowsRelayWorker::RelayContext : TqRelayBufferBudget {
    ...
    std::atomic<bool> StreamShutdownComplete{false};
    std::atomic<bool> StreamDetached{false};
    std::atomic<bool> TcpWriteShutdownQueued{false};
...
TqWindowsRelayWorkerSnapshot TqWindowsRelayWorker::BuildSnapshotLocal() const {
    ...
    for (const auto& relay : relays) {
        ...
        active.StreamDetached =
            relay->StreamDetached.load(std::memory_order_relaxed) ||
            relay->StreamShutdownComplete.load(std::memory_order_relaxed) ||
...
QUIC_STATUS TqWindowsRelayWorker::WindowsShutdownSinkTarget::OnStreamEvent(
    MsQuicStream* /*stream*/,
    QUIC_STREAM_EVENT* event,
    uint64_t /*routeGeneration*/) noexcept {
    if (event == nullptr || !SinkContext) {
        return QUIC_STATUS_SUCCESS;
    }
    if (event->Type == QUIC_STREAM_EVENT_SHUTDOWN_COMPLETE) {
        PublishStopToControl(SinkContext->StopControl, SinkContext->ControlGeneration);
    }
    return QUIC_STATUS_SUCCESS;
}

void TqWindowsRelayWorker::SwitchRelayTargetToShutdownSink(
    const std::shared_ptr<RelayContext>& relay) {
    if (!relay || relay->StreamOwner == nullptr) {
        return;
    }
    auto sinkContext = std::make_shared<WindowsShutdownSinkTarget::Context>();
    ...
    (void)relay->StreamOwner->PublishTarget(routeGeneration, std::move(sink));
    if (relay->ManagedBinding) {
        relay->ManagedBinding->RelayHint.store(nullptr, std::memory_order_release);
        relay->ManagedBinding->Closing.store(true, std::memory_order_release);
    }
    relay->StreamBinding = nullptr;
}

void TqWindowsRelayWorker::FlushPendingQuicReceivesOnClose(
    const std::shared_ptr<RelayContext>& relay) {
    if (!relay) {
        return;
    }
    ...
    const bool discard = ShouldDiscardReceiveView(relay->StreamOwner);
...
void TqWindowsRelayWorker::NudgeClosingRelayRetirement() {
    std::vector<std::shared_ptr<RelayContext>> relays;
    {
        ...
        relays.reserve(Relays_.size());
        for (const auto& entry : Relays_) {
            if (entry.second && entry.second->Closing.load(std::memory_order_acquire)) {
                relays.push_back(entry.second);
            }
        }
    }
...
bool TqWindowsRelayWorker::WorkerIocpOwnershipIsZero() const {
    ...
    return MaintenanceQueue_.empty();
}

void TqWindowsRelayWorker::BeginWorkerStopOnWorkerThread() {
    if (WorkerStopBeginDone_.exchange(true, std::memory_order_acq_rel)) {
        return;
    }
    ...
    for (const auto& relay : relays) {
        SwitchRelayTargetToShutdownSink(relay);
        FlushPendingQuicReceivesOnClose(relay);
...
bool TqWindowsRelayWorker::TestInjectTcpIocpCompletionForTest(
    ...
    if (!relay || Iocp_ == nullptr) {
        return false;
    }
    auto op = std::make_unique<IoOperation>();
    op->Event = tcpRecv ? TqWindowsIocpOperationType::TcpRecv
...
uint32_t TqWindowsRelayWorker::TestGetInFlightTcpRecvsForTest(uint64_t relayId) const {
    std::lock_guard<std::mutex> guard(Lock_);
    const auto it = Relays_.find(relayId);
    if (it == Relays_.end() || !it->second) {
        return 0;
    }
    return it->second->InFlightTcpRecvs.load(std::memory_order_acquire);
}

uint32_t TqWindowsRelayWorker::TestGetInFlightTcpSendsForTest(uint64_t relayId) const {
    std::lock_guard<std::mutex> guard(Lock_);
    const auto it = Relays_.find(relayId);
    if (it == Relays_.end() || !it->second) {
        return 0;
...
void TqWindowsRelayWorker::DrainRelayReceives(const std::shared_ptr<RelayContext>& relay) {
    ...
    for (;;) {
        ...
        if (relay->Closing.load(std::memory_order_acquire)) {
            ...
            if (!CompletePendingQuicReceive(relay, view)) {
                return;
            }
            continue;
        }
        ...
        if (!posted) {
            ...
            CloseRelay(
                ...
                TqRelayCloseMode::GracefulDrain,
                TqWindowsStreamCloseDisposition::ActiveShutdown,
                "post_tcp_send_receive_drain_failed");
        }
        return;
    }
}
    ...
    for (;;) {
        ...
        if (overlapped == nullptr) {
            ...
            if (Stopping_.load(std::memory_order_acquire)) {
                NudgeClosingRelayRetirement();
                if (WorkerIocpOwnershipIsZero()) {
                    FinalizeWorkerStopOnWorkerThread();
                    break;
                }
            }
            continue;
        ...
        case TqWindowsIocpOperationType::RelayReceiveReady:
            if (op->ReceiveView &&
                ...
                ActiveCompleteRemainingReceiveOwnership(relay, *op->ReceiveView);
...
void TqWindowsRelayWorker::RequestRelayShutdown(
    ...
    if (relay == nullptr ||
        relay->StreamShutdownComplete.load(std::memory_order_acquire)) {
        return;
    }
    auto lease = TryRelayStreamLease(*relay);
    if (relay->StreamOwner != nullptr && lease) {
...
bool TqWindowsRelayWorker::PostTerminalShutdownComplete(
    ...
    if (!binding || binding->RelayId == 0) {
        CallbackIocpPostFailedCount_.fetch_add(1, std::memory_order_relaxed);
        WindowsTerminalIocpPostFailedCount_.fetch_add(1, std::memory_order_relaxed);
        return false;
    }
    if (binding->TerminalShutdownPosted.exchange(true, std::memory_order_acq_rel)) {
        return true;
    }
    ...
    bool handoffToSink = false;
    {
        ...
        if (Iocp_ == nullptr || Stopping_.load(std::memory_order_acquire)) {
...
#if defined(TQ_UNIT_TESTING)
        else if (ForceTerminalIocpPostFailureForTest_.exchange(
            ...
            if (!::PostQueuedCompletionStatus(
                    static_cast<HANDLE>(Iocp_), 0, 0, &raw->Overlapped)) {
...
#if defined(TQ_UNIT_TESTING)
                {
                    ...
                    PostedCallbackSequence_.push_back(
    ...
    if (handoffToSink) {
        CallbackIocpPostFailedCount_.fetch_add(1, std::memory_order_relaxed);
        WindowsTerminalIocpPostFailedCount_.fetch_add(1, std::memory_order_relaxed);
        HandoffTerminalCleanupToShutdownSink(std::move(record));
    }
    return false;
}

void TqWindowsRelayWorker::HandoffTerminalCleanupToShutdownSink(
    std::shared_ptr<TerminalCleanupRecord> record) {
    if (!record) {
        return;
    }
    PublishStopToControl(record->StopControl, record->ControlGeneration);
    if (!record->Relay && record->RelayId != 0) {
        record->Relay = FindRelayByIdLocal(record->RelayId);
        if (record->Relay && record->Relay->Generation != record->Generation) {
            record->Relay.reset();
        }
    }
    {
        std::lock_guard<std::mutex> guard(TerminalShutdownSinkLock_);
        TerminalShutdownSink_.push_back(std::move(record));
        TerminalShutdownSinkPendingCount_.fetch_add(1, std::memory_order_acq_rel);
    }
    if (Stopping_.load(std::memory_order_acquire) &&
        WorkerThreadToken_.load(std::memory_order_acquire) == CurrentThreadToken()) {
        DrainTerminalShutdownSink();
    ...
    RequestTerminalShutdownSinkDrain();
}

void TqWindowsRelayWorker::RequestTerminalShutdownSinkDrain() {
    if (Stopping_.load(std::memory_order_acquire)) {
        if (WorkerThreadToken_.load(std::memory_order_acquire) == CurrentThreadToken()) {
            DrainTerminalShutdownSink();
        }
    ...
    auto op = std::make_unique<IoOperation>();
    op->Event = TqWindowsIocpOperationType::TerminalShutdownSinkDrain;
    IoOperation* raw = op.release();
    if (!::PostQueuedCompletionStatus(static_cast<HANDLE>(Iocp_), 0, 0, &raw->Overlapped)) {
        delete raw;
    }
...
void TqWindowsRelayWorker::DrainTerminalShutdownSink() {
    std::deque<std::shared_ptr<TerminalCleanupRecord>> pending;
    {
        std::lock_guard<std::mutex> guard(TerminalShutdownSinkLock_);
        pending.swap(TerminalShutdownSink_);
    }
    while (!pending.empty()) {
        auto record = std::move(pending.front());
        pending.pop_front();
        TerminalShutdownSinkPendingCount_.fetch_sub(1, std::memory_order_acq_rel);
        ProcessTerminalShutdownComplete(*record, false);
    }
}
...
void TqWindowsRelayWorker::ProcessTerminalShutdownComplete(
    ...
    bool decrementPendingCounter) {
    if (decrementPendingCounter) {
        TerminalOperationPendingCount_.fetch_sub(1, std::memory_order_acq_rel);
    }
#if defined(TQ_UNIT_TESTING)
    {
    ...
    if (!ValidateTerminalShutdownOperation(relay, record.Generation, record.ControlGeneration)) {
        PostedCallbackStaleDropCount_.fetch_add(1, std::memory_order_relaxed);
        auto control = std::atomic_load(&record.StopControl);
        PublishStopToControl(control, record.ControlGeneration);
        return;
...
void TqWindowsRelayWorker::TestForceNextTerminalIocpPostFailureForTest() {
    ForceTerminalIocpPostFailureForTest_.store(true, std::memory_order_release);
}

uint32_t TqWindowsRelayWorker::TestGetTerminalShutdownSinkPendingForTest() const {
    return TerminalShutdownSinkPendingCount_.load(std::memory_order_acquire);
}

void TqWindowsRelayWorker::TestDrainTerminalShutdownSinkForTest() {
    DrainTerminalShutdownSink();
}
...
bool TqWindowsRelayWorker::TestGetRelayStreamTerminalStateForTest(
    uint64_t relayId,
    bool* streamShutdownComplete,
    bool* streamDetached) const {
    if (streamShutdownComplete == nullptr || streamDetached == nullptr) {
        return false;
    }
    *streamShutdownComplete = false;
    ...
    *streamShutdownComplete =
        relay->StreamShutdownComplete.load(std::memory_order_acquire);
    *streamDetached = relay->StreamDetached.load(std::memory_order_acquire);
    return true;
}
...
size_t TqWindowsRelayWorker::TestCountPostedCallbackTypeForTest(
    TqWindowsIocpOperationType type) const {
    std::lock_guard<std::mutex> guard(LastPostedCallbackLock_);
    size_t count = 0;
    for (TqWindowsIocpOperationType posted : PostedCallbackSequence_) {
        if (posted == type) {
            ++count;
        }
    }
...
bool TqWindowsRelayWorker::TestSetBindingClosingForTest(uint64_t relayId, bool closing) {
    std::shared_ptr<WindowsStreamRelayBinding> binding;
    {
        std::lock_guard<std::mutex> guard(Lock_);
        const auto it = Relays_.find(relayId);
    ...
    binding->Closing.store(closing, std::memory_order_release);
    return true;
}
...
bool TqWindowsRelayWorker::TestReleaseQuicSendInFlightForRetirement(uint64_t relayId) {
    std::shared_ptr<RelayContext> relay;
    {
        std::lock_guard<std::mutex> guard(Lock_);
        const auto it = Relays_.find(relayId);
...
void TqWindowsRelayWorker::CloseRelay(
    ...
    const bool alreadyClosing = relay->Closing.load(std::memory_order_acquire);
    if (disposition != TqWindowsStreamCloseDisposition::TerminalLogicalDetach) {
        if (relay->StreamShutdownComplete.load(std::memory_order_acquire) || alreadyClosing) {
            TryRetireRelay(relay, traceState);
            return;
        }
    } else if (relay->StreamShutdownComplete.load(std::memory_order_acquire) &&
               alreadyClosing && !TqSocketValid(relay->TcpFd)) {
        TryRetireRelay(relay, traceState);
        return;
    }
    ...
    if (traceState && !alreadyClosing) {
        TqTraceRelayStopCondition("windows", WorkerIndex_, closeReason, BuildRelayTraceState(relay));
    }
    switch (disposition) {
    case TqWindowsStreamCloseDisposition::TerminalLogicalDetach:
        ApplyTerminalLogicalDetach(relay);
        break;
    case TqWindowsStreamCloseDisposition::ActiveAbort:
        ...
        break;
    case TqWindowsStreamCloseDisposition::ActiveShutdown:
        RequestActiveStreamShutdown(
            relay,
            disposition,
            mode == TqRelayCloseMode::AbortReset
                ? TqStreamLifetime::ShutdownIntent::AbortBoth
    ...
    if (TqSocketValid(relay->TcpFd)) {
        BeginTcpCancellation(relay, mode);
    }
    if (relay->ManagedBinding &&
        disposition != TqWindowsStreamCloseDisposition::TerminalLogicalDetach) {
        relay->ManagedBinding->Closing.store(true, std::memory_order_release);
    }
    if (disposition == TqWindowsStreamCloseDisposition::TerminalLogicalDetach) {
...
bool TqWindowsRelayWorker::ValidateTerminalShutdownOperation(
    const std::shared_ptr<RelayContext>& relay,
    uint64_t generation,
    ...
    return ControlGenerationMatches(relay->StopControl, controlGeneration);
}

void TqWindowsRelayWorker::ApplyTerminalLogicalDetach(
    const std::shared_ptr<RelayContext>& relay) {
    if (!relay) {
        return;
    }
...
void TqWindowsRelayWorker::RequestActiveStreamShutdown(
    const std::shared_ptr<RelayContext>& relay,
    TqWindowsStreamCloseDisposition disposition,
    TqStreamLifetime::ShutdownIntent intent) {
    if (!relay ||
    ...
    RequestRelayShutdown(relay, intent);
}

void TqWindowsRelayWorker::BeginTcpCancellation(
    const std::shared_ptr<RelayContext>& relay,
    TqRelayCloseMode mode) {
    if (!relay || !TqSocketValid(relay->TcpFd)) {
        return;
...
void TqWindowsRelayWorker::FailRelayFatal(const std::shared_ptr<RelayContext>& relay, const char* reason) {
    ...
    CloseRelay(
        relay,
        TqRelayCloseMode::AbortReset,
        TqWindowsStreamCloseDisposition::ActiveAbort,
        reason);
}
...
void TqWindowsRelayWorker::ProcessQuicPeerSendShutdown(
    const std::shared_ptr<RelayContext>& relay) {
    if (!relay || relay->StreamShutdownComplete.load(std::memory_order_acquire)) {
        return;
    }
    relay->TcpWriteShutdownQueued.store(true, std::memory_order_release);
    if (relay->PendingReceives.empty() &&
        relay->PendingQuicReceiveQueueDepth.load(std::memory_order_acquire) == 0 &&
        relay->PendingQuicReceiveBytes.load(std::memory_order_acquire) == 0) {
    ...
    if (relay->Compressor != nullptr) {
        ...
        if (compressed.empty() && !relay->Compressor->Flush(compressed)) {
            CloseRelay(
                ...
                TqRelayCloseMode::GracefulDrain,
                TqWindowsStreamCloseDisposition::ActiveShutdown,
                "compress_tcp_recv_flush_failed");
            return;
        }
        op->BufferOwner.reset();
        op->Buffer.clear();
            ...
            if (!MaybePostTcpRecv(relay) && !relay->Closing.load(std::memory_order_acquire)) {
                if (relay->TcpRecvClosed.load(std::memory_order_acquire) ||
                    ...
                    CloseRelay(
                        ...
                        TqRelayCloseMode::GracefulDrain,
                        TqWindowsStreamCloseDisposition::ActiveShutdown,
                        "tcp_recv_empty_compress_closed");
                } else {
...
void TqWindowsRelayWorker::ReleasePendingReceiveAccounting(
    ...
    if (relay && !view.AccountingReleased) {
        SaturatingFetchSub(relay->PendingQuicReceiveQueueDepth, 1);
        view.AccountingReleased = true;
    }
    view.Drained = true;
    view.CompletedLength = view.TotalLength;
    view.PendingCompleteBytes = 0;
            ...
            if (!(relay->Decompressor != nullptr && relay->CompressAlgo == TqCompressAlgo::Zstd)) {
                ...
                if (!PostTcpSendFromReceiveView(relay, view)) {
                    CloseRelay(
                        ...
                        TqRelayCloseMode::GracefulDrain,
                        TqWindowsStreamCloseDisposition::ActiveShutdown,
                        "post_tcp_send_receive_view_failed");
                    return;
                }
        ...
        if (relay->Decompressor != nullptr && relay->CompressAlgo == TqCompressAlgo::Zstd) {
            if (!PostTcpSendFromCompressedReceiveView(relay, view)) {
                CloseRelay(
                    ...
                    TqRelayCloseMode::GracefulDrain,
                    TqWindowsStreamCloseDisposition::ActiveShutdown,
                    "post_tcp_send_compressed_receive_view_failed");
                return;
            }
        } else {
            AdvanceReceiveView(relay, *view, bytes);
...
#endif
        // Router/registry claim already rewrote ClientContext to the claimed
        // delivered operation pointer; this is not an opaque envelope key.

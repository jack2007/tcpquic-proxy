...
public:
    TqBlockingDemoRelay(
        ...
        TqBlockingDemoRelayHandle* handle,
    ...
    TqBlockingDemoRelay(const TqBlockingDemoRelay&) = delete;
    TqBlockingDemoRelay& operator=(const TqBlockingDemoRelay&) = delete;
    ...
    bool Start() {
        ...
        try {
            ...
            TcpThread = std::thread(&TqBlockingDemoRelay::TcpToStreamLoop, this);
        } catch (...) {
...
private:
    static QUIC_STATUS QUIC_API StreamCallback(
        ...
        auto* relay = static_cast<TqBlockingDemoRelay*>(context);
        if (relay == nullptr) {

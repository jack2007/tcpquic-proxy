...
bool WaitForTcpRecvInFlightZero(
    TqWindowsRelayWorker& worker,
    uint64_t relayId,
    uint32_t timeoutMs);
bool WaitForTcpSendInFlightZero(
    TqWindowsRelayWorker& worker,
    uint64_t relayId,
    uint32_t timeoutMs);
...
bool RelaySendCountersZero(
    const TqWindowsRelayWorkerSnapshot& snapshot,
    uint64_t relayId) {
    for (const auto& active : snapshot.ActiveRelayStates) {
        if (active.RelayId == relayId) {
            return active.InFlightQuicSends == 0 &&
                active.OutstandingQuicSendBytes == 0;
        }
    }
    return snapshot.ActiveRelays == 0;
}

bool WaitForRelaySendCountersZero(
    TqWindowsRelayWorker& worker,
    uint64_t relayId,
    int timeoutMs) {
    ...
    do {
        if (RelaySendCountersZero(worker.Snapshot(), relayId)) {
            return true;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    } while (std::chrono::steady_clock::now() < deadline);
    return RelaySendCountersZero(worker.Snapshot(), relayId);
}
...
bool AssertWorkerLocalLifecycleZeroForTest(TqWindowsRelayWorker& worker) {
    ...
    return snapshot.ActiveRelays == 0 &&
        ...
        snapshot.PendingOverlappedTcpRecvs == 0 &&
        snapshot.PendingOverlappedTcpSends == 0 &&
        snapshot.PendingOverlappedWorkerOps == 0 &&
        snapshot.TerminalShutdownSinkPendingCount == 0 &&
        snapshot.TerminalOperationPendingCount == 0;
}

bool AssertWorkerLifecycleZeroForTest(TqWindowsRelayWorker& worker) {
    ...
    return AssertWorkerLocalLifecycleZeroForTest(worker) &&
        retained.OwnerCount == 0 &&
        registries.SendCompletionCount == 0;
}

bool StopRelayWorkerAndAssertZeroForTest(TqWindowsRelayWorker& worker) {
...
void ReleaseTestStreamOwnerForLifecycleAssert(
    ...
    if (owner->GetPhase() != TqStreamLifetime::Phase::TerminalPublished &&
...
bool RelayWorkerStopSignaled(const TqRelayHandle& handle) {
    const auto control = handle.Control;
    if (control != nullptr) {
        return control->Stop.load(std::memory_order_acquire);
    }
    return handle.Stop.load(std::memory_order_acquire);
}
...
int TestWindowsRelayQuicTeardownOnWorker() {
    {
        ...
        do {
            ...
            if (after.GracefulRelayDrains > before.GracefulRelayDrains &&
                RelayWorkerStopSignaled(handle)) {
                break;
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
        } while (std::chrono::steady_clock::now() < deadline);
    ...
    }
    {
        ...
        if (!RelayWorkerStopSignaled(handle) || snapshot.FatalRelayResets != 0) {
            receiveWorker.Stop();
            return 161;
        }
        receiveWorker.Stop();
    }
    {
        ...
        if (snapshot.FatalRelayResets != 0 || !RelayWorkerStopSignaled(handle)) {
            receiveWorker.Stop();
            return 163;
        }
        receiveWorker.Stop();
    }
    {
        ...
        if (!receiveWorker.TestHandleTcpPostFailureForTest(relayId, WSAECONNABORTED)) {
            (void)receiveWorker.TestReleaseTcpSendInFlightForRetirement(relayId);
            receiveWorker.Stop();
            return 232;
        }
        ...
        if (!receiveWorker.TestInjectTcpIocpCompletionForTest(
                relayId, false, 0, ERROR_OPERATION_ABORTED) ||
            !WaitForTcpSendInFlightZero(receiveWorker, relayId, 2000)) {
            (void)receiveWorker.TestReleaseTcpSendInFlightForRetirement(relayId);
            receiveWorker.Stop();
            return 235;
        }
        ...
        while (!RelayWorkerStopSignaled(handle) &&
    ...
    }
    {
        ...
        (void)receiveWorker.TestReleaseQuicSendInFlightForRetirement(relayId);
        receiveWorker.StopRelay(relayId);
        const auto drainDeadline =
            std::chrono::steady_clock::now() + std::chrono::milliseconds(2000);
        while (receiveWorker.Snapshot().ActiveRelays != 0 &&
...
bool TestWindowsRelayTraceContextDropsWhenClosing() {
    ...
    const auto staleDeadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(2000);
    uint64_t staleAfter = staleBefore;
    do {
        staleAfter = worker.Snapshot().WindowsPostedTraceContextStaleDropCount;
...
bool TestWindowsRelayLifecycleMetricsObservabilityForTest() {
    TqWindowsRelayWorker worker;
    if (!StartRelayWorkerForTest(worker)) {
        return false;
    }
    alignas(MsQuicStream) unsigned char streamStorage[sizeof(MsQuicStream)]{};
    auto* stream = reinterpret_cast<MsQuicStream*>(streamStorage);
    stream->Handle = reinterpret_cast<HQUIC>(static_cast<uintptr_t>(99));
    auto owner = MakeStartedRelayStreamOwner(stream);
    if (!owner) {
        worker.Stop();
        return false;
    ...
    const TqWindowsRelayWorkerSnapshot snapshot = worker.Snapshot();
    if (snapshot.WindowsTerminalSealCount == 0 ||
        snapshot.WindowsTerminalIocpPostCount == 0 ||
        snapshot.WindowsTerminalLogicalDetachCount == 0) {
        worker.Stop();
        return false;
    }
...
bool TestWindowsRelayZeroFinTerminalWinsForTest() {
    QUIC_API_TABLE fakeApi{};
    fakeApi.StreamReceiveComplete = FakeStreamReceiveComplete;
    fakeApi.StreamReceiveSetEnabled = FakeStreamReceiveSetEnabled;
    MsQuic = reinterpret_cast<const MsQuicApi*>(&fakeApi);
...
bool TestWindowsRelayZeroFinLeaseRetryForTest() {
    ...
    owner->DenyReceiveApiLeasesForTest(2);
...
bool TestWindowsRelayZeroFinOrphanGenerationMismatchForTest() {
    QUIC_API_TABLE fakeApi{};
    fakeApi.StreamReceiveComplete = FakeStreamReceiveComplete;
    fakeApi.StreamReceiveSetEnabled = FakeStreamReceiveSetEnabled;
    MsQuic = reinterpret_cast<const MsQuicApi*>(&fakeApi);
    ...
    const TqWindowsRelayWorkerSnapshot beforePost = worker.Snapshot();
    QUIC_STREAM_EVENT receive{};
    receive.Type = QUIC_STREAM_EVENT_RECEIVE;
    receive.RECEIVE.BufferCount = 0;
    receive.RECEIVE.Buffers = nullptr;
    ...
    if (!worker.TestBumpRelayGenerationForTest(handle.WindowsRelayId, 1)) {
        worker.Stop();
        MsQuic = nullptr;
        return false;
    }
    ...
    // New registration must not inherit old zero-FIN debt.
    alignas(MsQuicStream) unsigned char streamStorage2[sizeof(MsQuicStream)]{};
    auto* stream2 = reinterpret_cast<MsQuicStream*>(streamStorage2);
    stream2->Callback = MsQuicStream::NoOpCallback;
    stream2->Context = nullptr;
    stream2->Handle = reinterpret_cast<HQUIC>(static_cast<uintptr_t>(2));
    TqRelayHandle handle2{};
    if (!worker.RegisterRelayForTest(stream2, &handle2, tuning, TqCompressAlgo::None)) {
        worker.Stop();
        MsQuic = nullptr;
        return false;
    }
    const TqWindowsRelayWorkerSnapshot afterNew = worker.Snapshot();
    auto owner2 = worker.LastTestStreamOwnerForTest();
    const bool newRelayClean =
        afterNew.PendingQuicReceiveQueueDepth == 0 &&
        afterNew.PendingQuicReceiveBytes == 0 &&
        handle2.WindowsRelayId != handle.WindowsRelayId;

    FinalizeTestStreamOwnerForGateTeardownForTest(worker, owner);
    FinalizeTestStreamOwnerForGateTeardownForTest(worker, owner2);
    owner.reset();
    owner2.reset();
    worker.Stop();
    TqStreamLifetime::ResetLifecycleRegistriesForTest();
    MsQuic = nullptr;
    return newRelayClean;
}
...
int RunWindowsRelayTask5GateTestsForTest() {
    ...
    if (!TestWindowsRelayZeroFinTerminalWinsForTest()) {
        return 6120;
    }
    if (!TestWindowsRelayZeroFinLeaseRetryForTest()) {
        return 6121;
    }
    if (!TestWindowsRelayZeroFinOrphanGenerationMismatchForTest()) {
        return 6122;
    }
    if (!TestWindowsRelayZeroFinStopDrainSettlesForTest()) {
        return 6123;
    }
    {
...
bool WaitForTcpRecvInFlightZero(
    ...
    return worker.TestGetInFlightTcpRecvsForTest(relayId) == 0;
}
...
bool WaitForTcpSendInFlightZero(
    ...
    const auto deadline =
        std::chrono::steady_clock::now() + std::chrono::milliseconds(timeoutMs);
    while (std::chrono::steady_clock::now() < deadline) {
        if (worker.TestGetInFlightTcpSendsForTest(relayId) == 0) {
            return true;
        }
        (void)worker.Snapshot();
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
    return worker.TestGetInFlightTcpSendsForTest(relayId) == 0;
}

void DrainRelayTcpOwnershipForTest(TqWindowsRelayWorker& worker, uint64_t relayId) {
    for (int attempt = 0; attempt < 8; ++attempt) {
        if (worker.TestGetInFlightTcpRecvsForTest(relayId) > 0) {
            (void)worker.TestInjectTcpIocpCompletionForTest(
                relayId, true, 0, ERROR_OPERATION_ABORTED);
            (void)WaitForTcpRecvInFlightZero(worker, relayId, 500);
        }
        if (worker.TestGetInFlightTcpSendsForTest(relayId) > 0) {
            (void)worker.TestInjectTcpIocpCompletionForTest(
                relayId, false, 0, ERROR_OPERATION_ABORTED);
            (void)WaitForTcpSendInFlightZero(worker, relayId, 500);
        }
        if (worker.TestGetInFlightTcpRecvsForTest(relayId) == 0 &&
...
int RunWindowsRelayTask6GateTestsForTest() {
    TqStreamLifetime::ResetLifecycleRegistriesForTest();
    {
        ...
        worker.Stop();
        {
            const TqWindowsRelayWorkerSnapshot snap = worker.Snapshot();
            if (worker.TestHasIocpForTest() || snap.ActiveRelays != 0 ||
                snap.TcpCancelRequestedCount == 0) {
                TqCloseSocket(pair[1]);
                return 605;
            }
        }
    ...
    }
    {
        ...
        stream->Callback = MsQuicStream::NoOpCallback;
        stream->Context = nullptr;
        stream->Handle = reinterpret_cast<HQUIC>(static_cast<uintptr_t>(602));
        TqRelayHandle handle{};
        TqTuningConfig tuning{};
        tuning.RelayIoSize = 4096;
        if (!worker.RegisterRelay(
        ...
        if (!worker.TestInjectTcpIocpCompletionForTest(
                relayId, true, 0, ERROR_OPERATION_ABORTED)) {
            worker.Stop();
            TqCloseSocket(pair[1]);
            return 611;
        }
        if (!WaitForTcpRecvInFlightZero(worker, relayId, 2000)) {
            worker.Stop();
            TqCloseSocket(pair[1]);
            return 612;
        }
    ...
    }
    {
        ...
        stream->Callback = MsQuicStream::NoOpCallback;
        stream->Context = nullptr;
        stream->Handle = reinterpret_cast<HQUIC>(static_cast<uintptr_t>(603));
        TqRelayHandle handle{};
        TqTuningConfig tuning{};
        tuning.RelayIoSize = 4096;
        if (!worker.RegisterRelayForTest(stream, &handle, tuning, TqCompressAlgo::None)) {
        ...
        const uint64_t relayId = handle.WindowsRelayId;
        if (!worker.TestMarkTcpSendInFlightForTest(relayId)) {
            worker.Stop();
            return 616;
        }
        if (!worker.TestInjectTcpIocpCompletionForTest(
                relayId, false, 0, ERROR_OPERATION_ABORTED)) {
            (void)worker.TestReleaseTcpSendInFlightForRetirement(relayId);
            worker.Stop();
            return 617;
        }
        if (!WaitForTcpSendInFlightZero(worker, relayId, 2000)) {
            (void)worker.TestReleaseTcpSendInFlightForRetirement(relayId);
            worker.Stop();
            return 618;
        }
    ...
    }
    {
        ...
        stream->Callback = MsQuicStream::NoOpCallback;
        stream->Context = nullptr;
        stream->Handle = reinterpret_cast<HQUIC>(static_cast<uintptr_t>(604));
        TqRelayHandle handle{};
        TqTuningConfig tuning{};
        tuning.RelayIoSize = 4096;
        if (!worker.RegisterRelayForTest(stream, &handle, tuning, TqCompressAlgo::None)) {
        ...
        if (!worker.TestMarkTcpSendInFlightForTest(relayId) ||
            !worker.TestMarkTcpRecvInFlightForRetirement(relayId)) {
            worker.Stop();
            return 623;
        }
        if (!worker.TestInjectTcpIocpCompletionForTest(
    ...
    }
    {
        ...
        stream->Callback = MsQuicStream::NoOpCallback;
        stream->Context = nullptr;
        stream->Handle = reinterpret_cast<HQUIC>(static_cast<uintptr_t>(605));
        TqRelayHandle handle{};
        TqTuningConfig tuning{};
        tuning.RelayIoSize = 4096;
        if (!worker.RegisterRelayForTest(stream, &handle, tuning, TqCompressAlgo::None)) {
    ...
    }
    {
        ...
        stream->Callback = MsQuicStream::NoOpCallback;
        stream->Context = nullptr;
        stream->Handle = reinterpret_cast<HQUIC>(static_cast<uintptr_t>(606));
        TqRelayHandle handle{};
        TqTuningConfig tuning{};
        tuning.RelayIoSize = 4096;
        if (!worker.RegisterRelayForTest(stream, &handle, tuning, TqCompressAlgo::None)) {
    ...
    {
        {
            ...
            stream->Callback = MsQuicStream::NoOpCallback;
            stream->Context = nullptr;
            stream->Handle = reinterpret_cast<HQUIC>(static_cast<uintptr_t>(607));
            if (!worker.TestArmStopDrainForTest()) {
                worker.Stop();
                return 644;
            }
            TqRelayHandle handle{};
...
#if 0  // pending-register wake crashes — covered by sync reject above
        {
            ...
            stream->Callback = MsQuicStream::NoOpCallback;
            stream->Context = nullptr;
            stream->Handle = reinterpret_cast<HQUIC>(static_cast<uintptr_t>(608));
            TqWindowsRelayWorkerQueueBlockForTest block{};
            if (!worker.TestPostWorkerQueueBlockForTest(&block) ||
                !worker.TestWaitWorkerQueueBlockEnteredForTest(block, 2000)) {
                worker.TestReleaseWorkerQueueBlockForTest(block);
            ...
            std::atomic<bool> registerOk{true};
            std::thread registerThread([&] {
                TqRelayHandle handle{};
                TqTuningConfig tuning{};
                tuning.RelayIoSize = 4096;
                registerOk.store(
                    worker.RegisterRelayForTest(stream, &handle, tuning, TqCompressAlgo::None),
                    std::memory_order_release);
            ...
            registerThread.join();
            worker.Stop();
            if (!registerDone.load(std::memory_order_acquire) ||
                registerOk.load(std::memory_order_acquire) ||
                worker.TestHasIocpForTest()) {
                return 648;
            }
        }
#endif
    }
#if 0  // CancelIoEx(NOT_FOUND) metrics covered by stop-drain gate 605-613; async StopRelay+Stop can deadlock Snapshot.
    {
        TqSocketHandle pair[2]{TqInvalidSocket, TqInvalidSocket};
        if (!TqSocketPair(pair)) {
            return 649;
        ...
        stream->Callback = MsQuicStream::NoOpCallback;
        stream->Context = nullptr;
        stream->Handle = reinterpret_cast<HQUIC>(static_cast<uintptr_t>(609));
        TqRelayHandle handle{};
        TqTuningConfig tuning{};
        tuning.RelayIoSize = 4096;
        if (!worker.RegisterRelay(
        ...
        const TqWindowsRelayWorkerSnapshot afterStop = worker.Snapshot();
        if (afterStop.TcpCancelRequestedCount == 0 ||
            afterStop.TcpCancelNotFoundCount == 0) {
            TqCloseSocket(pair[1]);
            return 653;
        }
        TqCloseSocket(pair[1]);
...
int main() {
    ...
    }
    {
        ...
        auto owner = sendCompleteWorker.LastTestStreamOwnerForTest();
        if (!owner) {
            sendCompleteWorker.Stop();
            return 136;
        }
    ...
    }
    {
        ...
        if (!sendCompleteWorker.TestDrainSingleQuicSendCompleteForTest()) {
            sendCompleteWorker.Stop();
            return 408;
        }
        const TqWindowsRelayWorkerSnapshot snapshot = sendCompleteWorker.Snapshot();
        if (snapshot.Errors != 0 || snapshot.WindowsCallbackIocpPostFailedCount != 0 ||
            snapshot.FatalRelayResets != 0 || snapshot.ActiveRelayStates.size() != 1 ||
        ...
        if (!sendCompleteWorker.TestCompleteTcpRecvInFlightForRetirement(
                handle.WindowsRelayId)) {
            sendCompleteWorker.Stop();
            return 409;
        }
    ...
    }
    {
        ...
        receiveWorker.Stop();
        if (!AssertWorkerLocalLifecycleZeroForTest(receiveWorker)) {
            TqCloseSocket(pair[1]);
            MsQuic = nullptr;
            std::_Exit(6113);
        }
    ...
    }
    {
        ...
        snapshot = receiveWorker.Snapshot();
        if (!RelayWorkerStopSignaled(handle) || snapshot.ActiveRelays != 0) {
            receiveWorker.Stop();
            return 205;
        }
        receiveWorker.Stop();
    }
    {
        ...
        receiveWorker.StopRelay(relayId);
        if (!receiveWorker.TestReleaseQuicSendInFlightForRetirement(relayId)) {
            receiveWorker.Stop();
            return 208;
        }
        const auto retireDeadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(2000);
    ...
    }
    {
        ...
        const TqWindowsRelayWorkerSnapshot snapshot = receiveWorker.Snapshot();
        if (!RelayWorkerStopSignaled(handle) || g_StreamReceiveCompleteBytes != 5 ||
    ...
    }
    {
        ...
        if (!receiveWorker.TestArmStopDrainForTest()) {
            receiveWorker.Stop();
            MsQuic = nullptr;
            return 6100;
        }
        ...
        const TqWindowsRelayWorkerSnapshot postFailureSnapshot = receiveWorker.Snapshot();
        if (postFailureSnapshot.PendingQuicReceiveQueueDepth != 0) {
            receiveWorker.Stop();
            MsQuic = nullptr;
            std::_Exit(6103);
        }
...
#if 0  // Legacy backpressure/fatal-send teardown hangs without full relay drain under Task 6 stop order.
    {
        QUIC_API_TABLE fakeApi{};
        fakeApi.StreamSend = FakeStreamSend;
        MsQuic = reinterpret_cast<const MsQuicApi*>(&fakeApi);
        ...
        g_FakeStreamSendStatus = QUIC_STATUS_SUCCESS;
        (void)TqShutdownSend(pair[1]);
        const uint64_t relayId = handle.WindowsRelayId;
        DrainRelayTcpOwnershipForTest(receiveWorker, relayId);
        (void)WaitForTerminalPendingClearedForTest(receiveWorker, 2000);
        receiveWorker.StopRelay(relayId);
        const auto relayDrainDeadline =
            std::chrono::steady_clock::now() + std::chrono::milliseconds(5000);
        while (receiveWorker.Snapshot().ActiveRelays != 0 &&
               std::chrono::steady_clock::now() < relayDrainDeadline) {
            (void)receiveWorker.Snapshot();
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }
        receiveWorker.Stop();
    ...
    }
    {
        ...
        g_FakeStreamSendStatus = QUIC_STATUS_SUCCESS;
        (void)TqShutdownSend(pair[1]);
        const uint64_t fatalRelayId = handle.WindowsRelayId;
        DrainRelayTcpOwnershipForTest(receiveWorker, fatalRelayId);
        receiveWorker.StopRelay(fatalRelayId);
        const auto fatalDrainDeadline =
            std::chrono::steady_clock::now() + std::chrono::milliseconds(5000);
        while (receiveWorker.Snapshot().ActiveRelays != 0 &&
               std::chrono::steady_clock::now() < fatalDrainDeadline) {
            (void)receiveWorker.Snapshot();
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }
        receiveWorker.Stop();
...
#endif
    {
        ...
        const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(2000);
        while (!RelayWorkerStopSignaled(handle) && std::chrono::steady_clock::now() < deadline) {
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }
        const TqWindowsRelayWorkerSnapshot snapshot = receiveWorker.Snapshot();
        if (!RelayWorkerStopSignaled(handle) || snapshot.RelayBufferBytesInUse != 0) {
            receiveWorker.Stop();
            TqCloseSocket(pair[1]);
            MsQuic = nullptr;
            return 100;
...
#endif
    {
        ...
        do {
            snapshot = receiveWorker.Snapshot();
            if (RelayWorkerStopSignaled(handle) && snapshot.GracefulRelayDrains > 0 &&

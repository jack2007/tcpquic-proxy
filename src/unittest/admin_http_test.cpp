...
int main() {
...

    {
        ...
        if (html.find("<label>connections</label><input id=\"peer-connections\" type=\"number\" min=\"1\" max=\"128\" value=\"1\">") == std::string_view::npos) return 326;
        if (js.find("document.getElementById('peer-connections').value = String(peer.proto_connections || peer.quic_connections || 1);") == std::string_view::npos) return 327;
        if (js.find("proto_connections: Number(document.getElementById('peer-connections').value || 1),") == std::string_view::npos) return 328;
        if (!TqCssRuleHasDeclaration(css, ".peer-form-grid", "grid-template-columns:repeat(3,minmax(180px,1fr))")) return 329;
        ...
        if (html.find("<strong>Delete peer</strong>") != std::string_view::npos) return 379;
        ...
        if (html.find("<section class=\"peer-form-panel hidden\" id=\"peer-form-panel\">") == std::string_view::npos) return 418;
        if (html.find("<tfoot><tr><td colspan=\"12\"></td><td><button class=\"btn primary\" id=\"peer-create\">Create Peer</button></td></tr></tfoot>") == std::string_view::npos) return 419;
        if (html.find("peer-create-footer") != std::string_view::npos) return 450;
        if (css.find(".peer-create-footer") != std::string_view::npos) return 451;
        ...
        if (html.find("<th>peer</th><th>client_name</th>") != std::string_view::npos) return 565;
        if (html.find("<th>connection_id</th><th>peer</th><th>client_name</th>") != std::string_view::npos) return 576;
        ...
        if (js.find("['peer','client_name','remote_address','connections','active_streams','total_streams_opened','active_tunnels','last_error']") != std::string_view::npos) return 579;

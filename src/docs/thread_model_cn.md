...
- terminal lifetime 指标已贯通：`terminal_seal/post/logical_detach`、terminal 后 API 抑制、`pending overlapped` recv/send、`CancelIoEx` 分类、stop drain 剩余量、terminal retention、send completion registry 和 shutdown sink 深度（见 `docs/relay_win.md` §8）。

Windows 相关计划仍是 active plan：源码已有统一 pending/zstd/metrics/terminal-lifetime 的实现基础，但 Windows 构建、unit、loopback 和双机验证需要在 Windows 机器上继续闭环。

### 6.2 Windows stream terminal 与 stop drain

- callback `SHUTDOWN_COMPLETE` seal target 并 post terminal IOCP record（无 stream pointer）；失败走 shutdown sink。
- worker `Stop()` 对每个 active relay 请求 shutdown、切 shutdown sink、cancel 本地 TCP，然后 GQCS drain 到 `InFlightTcp*`/`QueuedWorkerOps`/terminal pending 为零才关闭 IOCP。
- started owner 的 terminal retention 可在 worker 退出后继续由 router/shutdown sink 持有；worker 本地 `OVERLAPPED` 必须 drain 到零。

### 6.1 Windows QUIC -> TCP callback IOCP 调度

Windows `RECEIVE` callback 不再把 `QuicReceiveView(view*)` 投递到 worker event queue。callback 线程只负责把 receive view 放入 `relay->PendingReceives`，随后向 worker IOCP 投递 `RelayReceiveReady(relayId)`。这个 posted operation 只表示“该 relay 有 receive 可 drain”，不携带 receive view 指针。

worker 收到 `RelayReceiveReady(relayId)` 后，从 `relay->PendingReceives` 队首开始 drain：plain path 按 TCP send 进度写出 MsQuic receive slices，zstd path 先在 worker 中解压到 TCP send buffer，再 post `WSASend`。如果 TCP send completion、`FinishReceiveView()` 或 pending view 推进后仍有可继续处理的数据，会投递可合并的 `RelayReceiveDrain(relayId)`；同一个 relay 已经有 drain queued 时只合并计数，不重复排队。这样 worker event queue 不再作为 MsQuic callback 事件通道，receive 顺序由 pending queue 队首消费保证。
...
Darwin managed relay 使用同一套 public control / owner 契约，并额外固化：

- tunnel caller 只发布/清理 `TqRelayHandle`；worker、binding、callback、event queue 只持 `TqRelayControl` + immutable generation。
- registration 是 prepare / publish / commit 事务；publish 前完整初始化 binding identity，publish 后禁止 reset 同一 weak pointer。
- send completion 使用 move-only reservation；pre-submit 任一退出自动 cancel。
- RECEIVE allocation failure 只发布 `ActiveShutdown`，不伪造 terminal。
- worker stop 先安装 `ShutdownSink`，再退出 kqueue 线程；exited purge 不进入 worker-only 数据面 handler。

发布门禁 metrics（`relay_*` 中性字段）覆盖：active control、stop signaled、generation mismatch、accounting duplicate release、prepared/commit/rollback/activation、precommit bytes/depth、send reservation、pending receive/discard、active-failure by reason、shutdown sink、worker-exited purge、stop remaining/oldest age。multi-worker 聚合对全局 terminal retention / send reservation 使用 max，不对 worker 求和。
